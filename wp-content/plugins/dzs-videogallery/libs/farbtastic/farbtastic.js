    ...
    if (typeof fb.callback == 'object') {
      ...
      $(fb.callback).each(function() {
        if (this.value != fb.color) {
            ///console.info(fb.color, this);
            var aux = fb.color;
            if(String(this.className).indexOf('colorpicker-nohash') > -1){
                aux = aux.replace('#', '');
            }
          this.value = aux;
        }
...
window.farbtastic_reinit = function(){
    ...
    //console.info(jQuery('.with-colorpicker'));
    jQuery('.with-colorpicker').each(function(){
        ...
        // console.info(jQuery.fn.farbtastic, _t.next())
